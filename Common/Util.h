...
const int key_len = sizeof(key_type);

#define MAX_TRAFFIC 6

key_type str_to_key(const char* p) {
    key_type res;
    memcpy(&res, p, key_len);
    return res;
}

class SubsetCheck {
public:
    virtual bool operator() (key_type key) const = 0;
};

class SubsetCheckPrefix : public SubsetCheck {
public:
    key_type mask;
    key_type base_key;
    int prefix_len;

    SubsetCheckPrefix(key_type _base_key, int len) {
        mask = (((uint64_t)1 << len) - 1) << (32 - len);
        base_key = _base_key & mask;
        //cout << mask << ' ' << base_key << endl;
    }

    virtual bool operator() (key_type key) const {
        return (key & mask) == base_key;
    }
};
...
struct TUPLES {
    ...
    bool operator<(const TUPLES& t) const {
...
typedef std::unordered_map<key_type, uint32_t> HashMap;
